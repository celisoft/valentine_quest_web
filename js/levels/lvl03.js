function lvl03_create()
{
	...
	music.play('',0,1,true);
	this.game.add.image(0,0,'bg_grotoxien');
	...
	//Generating grounds
	generate_ground(0,16,0,'ground_grotoxien');
	generate_ground(0,4,3,'ground_grotoxien');
	generate_ground(7,13,3,'ground_grotoxien');
	generate_ground(3,8,5,'ground_grotoxien');
	generate_ground(0,4,7,'ground_grotoxien');
	generate_ground(0,9,9,'ground_grotoxien');
	generate_ground(0,16,11,'ground_grotoxien');

	//Integrate exit
	exit_door = this.game.add.sprite(2*64, 64, 'door_grotoxien');
	...
	var heart = hearts.create(4*64,8*64, 'heart');
	heart = hearts.create(7*64,64, 'heart');
	heart.visible = false;
	heart = hearts.create(9*64,10*64, 'heart');
	heart.visible = false;
	heart = hearts.create(8*64,4*64, 'heart');
	heart.visible = false;
	...
	var monster = game.add.sprite(6*64,64, 'monster');
	...
	monster = game.add.sprite(6*64,4*64, 'monster');
	monster.animations.add('moving', [ 1, 2], 1, true);
	monster.animations.play('moving');
	monsters.add(monster);
	monster_max = 10;
	...
	monster = game.add.sprite(0,8*64, 'monster');
	monster.animations.add('moving', [ 1, 2], 1, true);
	monster.animations.play('moving');
	monsters.add(monster);
	monster_max = 10;
	...
	//Integrate player
	player = game.add.sprite(64, 10*64, 'grotoxien');
